...
const MapComponent: React.FC = () => {
    ...
    useEffect(() => {
        ...
        axios
            .get<Playground[]>("http://localhost:8081/maps/playgrounds")
            .then((response) => {
        ...
        >
            <MapContainer center={[44.816667, 20.466667]} zoom={10} style={{ flexGrow: 1 }}>
                ...
                {filteredLocations.map((loc, index) => (
                    <Marker key={`${loc.email}-${index}`} position={[loc.latitude, loc.longitude]} icon={defaultIcon}>
                        <Popup>
                            <b>{loc.full_name || "Нет данных"}</b>
                            <br />
                            <b>Программа:</b> {loc.groups.map((g) => groupNames[g] || g).join(", ") || "Нет данных"}
                            <br />
                            <b>Email:</b> {loc.email}
                            <br />
                        ...
                        <Marker key={`pg-${pg.data_id}-${index}`} position={[pg.lat, pg.lng]} icon={playgroundIcon}>
                            <Popup>
                                <div className="popup-content">
                                    <b>📍Площадка📍</b>
                                    <br />
                                    <b>Покрытие:</b> {pg.pokritie || "Нет данных"}
                                    <br />
                                    <b>Дренаж:</b> {pg.drenaj || "Нет данных"}
                                    <br />
                                    <b>Ограждение:</b> {pg.ograjdenie || "Нет данных"}
                                    <br />
                                    ...
                                    <br />
                                    <b>
                                        <a href={pg.url_adress} target="_blank">
                                            Google Maps
                                        </a>
                                    </b>
                                </div>
