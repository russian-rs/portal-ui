...
const ADDRESS_FONT = { max: 9, min: 6, lineGapRatio: 0.40 };
...
export default async function generateEnvelopPdf(application: ApplicationDto) {
    ...
    const templateBytes = await fetch("/resources/envelop.pdf")
    ...
    function mapXY(xVis: number, yVis: number, fontSize: number, orientation: Ori) {
        ...
        let px = xVis, py = yTop; let rot: Rotation | undefined;

        switch (orientation) {
            case 0:   break;
            case 90:  px = yVis; py = vWidth  - xVis - fontSize; rot = degrees(90);  break;
            case 180: px = vWidth - xVis - fontSize; py = vHeight - yTop - fontSize; rot = degrees(180); break;
            case 270: px = vHeight - yVis - fontSize; py = xVis; rot = degrees(270); break;
        }
    ...
    function fitTextInBox(
        ...
        let lo = min, hi = max;
        let bestSize = min, bestLines = wrapText(font, min, text, rect.w);
